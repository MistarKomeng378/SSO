...
?>
			...
			<table border="1" cellpadding='3'>
				...
				<?php
						$no=1;
						$tbobot = 0;
						$query = mysql_query("SELECT DISTINCT id_srko,rencana_kerja,bobot,target,satuan FROM srko WHERE CostCenter='$getCC' AND tahun='$getTahun' AND parent_srko='' order by id_srko");
						while($r=mysql_fetch_array($query)){
							...
							if($prog['jenis_resume']==1){
								...
								$realisasi 	= desimal($jr11['realisasi']);
								$rea	 	= $jr11['realisasi'];
								...
								$tot_target = desimal($jr2['sumtarget']);
								$tt 		= $jr2['sumtarget'];
								...
								$realisasi 	= desimal($jr22['sumrealisasi']);
								...
								$tot_target = desimal($jr3['avgtarget']);
								$tt 		= $jr3['avgtarget'];
								...
								$realisasi 	= desimal($jr33['avgrealisasi']);
								...
								<td align='center' bgcolor='#99ff99' title='Realisasi - Bulanan'>
									";
										if($prog['jenis_pencapaian']==3){
											echo desimal($prog['realisasi']);
										}else{
											echo"$prog[realisasi]";
										}								
									echo"
								</td>
								<td align='center' bgcolor='#99ff99' title='Pencapaian - Bulanan'><span style=\"color:$fc2\">".desimal($prog['pencapaian'])." %</span></td>
