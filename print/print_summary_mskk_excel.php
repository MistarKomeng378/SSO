...
set_time_limit(0);
	...
	timeline($_SESSION['nik'],"download","Telah melakukan download Summary MSKK  $getUnit Tahun $getTahun");
	
	header("Content-type: application/octet-stream");
	header("Content-Disposition: attachment; filename=Summary_MSKK_".$getTahun.".xls");//ganti nama sesuai keperluan
	header("Pragma: no-cache");
	header("Expires: 0");
?>
<center>
	<h3>SUMMARY MSKK <?php $uraian=mysql_fetch_array(mysql_query("SELECT uraian FROM mskko WHERE CostCenter='$getUnit'")); if(!empty($getUnit)){echo"$uraian[uraian]";}?><br>
	PERIODE <?=$getTahun?>
	</h3>
				...
				echo"
					<tr>
						...
						for($i=1;$i<=12;$i++){
							...
							echo"<td align='center'>".desimal($JmlMskk['bxs'])."</td>";
						}
				echo"</tr>";
				$no++;				
			}
