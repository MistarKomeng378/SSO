...
?>			
			...
			<!--<h4 align="center"><b>Penilaian Sasaran Kerja Karyawan Tahun <?=$getTahun?> <br/> <?=$unit['uraian']?></b></h4>-->
			<?
			if($_SESSION['level']==4){
				if(isset($unit['CostCenter'])){
					...
					echo "
						<h4 align='center'><b>Penilaian Sasaran Kerja Karyawan Tahun $getTahun <br/> Penilai, ".$_SESSION['name']."</b></h4>	
					";
				}
				...
				<?php
						...
						while($r=mysql_fetch_array($query)){
								...
								// if(isset($unit['CostCenter'])){
									// $cc= mysql_fetch_array(mysql_query("select * from mskko where CostCenter='".$r['cc']."'"));
								// }else{
									// $cc= mysql_fetch_array(mysql_query("select * from mskko where CostCenter='".$r['divisi']."'"));	
								// }
						
							if($_SESSION['level']==4 OR 5){
								$nama		= name($r['nik']);
								if(isset($unit['CostCenter'])){
									$cc= mysql_fetch_array(mysql_query("select * from mskko where CostCenter='".$r['cc']."'"));
								}else{
