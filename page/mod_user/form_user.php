...
<!--------------------------------------------------------------------------------------------------------------------->
<?php
if(isset($_REQUEST['opt'])=="nik"){
	$getId		= mysql_real_escape_string(dc($_REQUEST['edit']));
	$edit 		= mysql_fetch_array(mysql_query("SELECT * FROM user WHERE id='$getId' "));
	$nik 		= $edit['nik'];
	$opt		= $_REQUEST['opt'];
?>
<form method="POST" action="page/mod_user/query_user.php?opt=<?=$opt?>" id="formku">
	
	<div class="col-lg-6">
		<div class="col-lg-12">
			<label for="nik">NIK</label>
			<input type="text" class="form-control required" name="nik" id="nik" placeholder="NIK" value="<?=$nik?>" readonly>
		</div>
		<div class="col-lg-12">
			<label for="nik_baru">NIK BARU</label>
			<input type="text" class="form-control required" name="nik_baru" id="nik_baru" placeholder="NIK BARU" value="" >
		</div>		
		<div class="col-lg-12">
		<hr>
		</div>
		<div class="col-lg-12">
			<div class="box-footer">
				...
				<button type="submit" name="" value="" class="btn btn-danger" onclick="history.back()">Batal</button>
			</div>
        </div>
	</div>
</form>
<?php
	...
	if(isset($_REQUEST['edit'])){
		...
		$email 		= $edit['email'];
		$pass 		= $edit['nik'];
		$edit2 		= mysql_fetch_array(mysql_query("SELECT * FROM m_karyawan WHERE regno='$nik' "));
		$poscode	= $edit2['poscode'];
		$dept		= $edit2['dept'];
		$opt		= "edit";
	}else{
					...
					<div class="form-group">
						<label for="unit">Unit</label>
							<select class="form-control" name="unit" id="dept" onchange="show(this.value)">
								...
								<?php
									$mskko = mysql_query("SELECT * FROM mskko where CostCenter !='KL'");
									while($r=mysql_fetch_array($mskko)){
										echo"<option value='$r[id]-$r[CostCenter]' "; if($r['id']==$grup_id){ echo"selected";} echo">$r[uraian] </option>";
									}
								?>
							</select>
					</div>
							...
							<select class="form-control" name="jab" id="jab" >
								...
								if(isset($_REQUEST['edit'])){
									...
									while($r=mysql_fetch_array($jab)){
										echo"<option value='$r[poscode]' "; if($r['poscode']==$poscode){ echo"selected";} echo">$r[posdesc] </option>";
									}
								}
								?>
							</select>
