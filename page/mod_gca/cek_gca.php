...
include"config/fungsi_timeline.php";
	...
	$kpi			= mysql_fetch_array(mysql_query("SELECT tahun,kode_kpi,icon FROM wbs WHERE id='$parentId' "));
	$tahun_aktif	= mysql_real_escape_string($kpi['tahun']);
if($jenisGCA==1){
...
/////////////////////////////////////////////////////////////
	...
	$intID		= substr($id_baru['id'], 3, 5);
	$strID		= substr($id_baru['id'], 0, 3);
	$id_lama	= $intID+1;
	$id_lama	=sprintf("%05s", $id_lama);
	$id_lama	= $strID.$id_lama;
	$jum1= count($_POST['pic']);
	$jmlpic = 0;
	
	// echo"--$jum1 = $id_lama<br>";
	for($a=0; $a<$jum1; $a++){
		$ppic = $_POST['pic'][$a];
		// echo"$ppic-$pic_asli<br>";
		if($ppic==$pic_asli){
			$jpic = 1;
		}else{
			$jpic = 0;
	...
	for($i=0; $i<$jum1; $i++){
		...
		if($_GET['opt']=="edit"){
			...
			timeline($nik,"edit","update GCA dengan ID_GCA -> $idBaru : $aktivitas dengan penanggung jawab $pic");	
			...
			mysql_query("REPLACE INTO `wbs` SET `id`		='$id',
											...
											`icon`			='$icon',
											`gca_by`		='$nik'");
			timeline($nik,"tambah","penambahan GCA dengan ID_GCA -> $id : $aktivitas dengan penanggung jawab $pic");
		}
