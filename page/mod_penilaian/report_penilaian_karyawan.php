...
?>	
		...
		<br>
		<? 
		// $ses = mysql_fetch_array(mysql_query("SELECT DISTINCT pm FROM penilaian_kerja where pm ='".$_SESSION['nik']."'"));		
		$ses = mysql_fetch_array(mysql_query("SELECT DISTINCT pm FROM penilaian_kerja where pm='".$_SESSION['nik']."'"));		
		if($_SESSION['level']==4){
			echo"		
			
			<div class='col-lg-3'>					
				<form method='POST' action='page/mod_penilaian/aksi_tahun_skk2.php'>
					<div class='col-lg-6'>
			...
			<div class='col-lg-2'>
				<a href='print/print_penilian_kerja_karyawan_excel.php?id=".ec($unit['CostCenter'])."-".ec($getTahun)."' target='_blank' class='btn btn-success'><i class='fa fa-print' ></i> Cetak Excel</a> 		
			</div>
			<table border='0' width='30%' align='right'>
			<tr >
				<td width='75%' height='50'>
				<form method='GET' action='".$_SERVER['PHP_SELF']."'>
					<input type='hidden' name='page' value='report_penilaian_karyawan'>
					<select name='unit' class='form-control'>
						<option value=''>Pilih Unit</option>
			";
					$qunit = mysql_query("SELECT * FROM mskko WHERE id !='1.6' AND id!='4' AND CostCenter='".$_SESSION['cc']."' order by id");
					...
					<?php
						...
						if($_SESSION['level']==4){
							if(isset($_GET['unit'])){
								...
								// $query = mysql_query("SELECT DISTINCT nik, divisi FROM penilaian_kerja where pm='".$_SESSION['nik']."' AND divisi ='".$getUnit."'");
							}else{						
						...
						while($r=mysql_fetch_array($query)){
							...
							if(isset($_GET['unit'])){
							    $cc_kar= mysql_fetch_array(mysql_query("select * from mskko where CostCenter='".$r['cc']."'"));
							}else{
									...
									if($getEdit==1){ 
										if($sumhasil['jum_hasil']==0){
											...
											<div class='stats-link'>
												<a href='#modal-mskk' title='Detile'  class='show-mskk'  data-id='".ec($r['nik'])."-".ec($r['name'])."-".ec($cc_kar['CostCenter'])."-".ec($getTahun)."' data-toggle='modal'><span class='btn btn-xs btn-primary'><i class='fa fa-search-plus'></i></span></a>
											</div>
										";
										}
									}
