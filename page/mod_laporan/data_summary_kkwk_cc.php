						...
						<select name='tahun' class='form-control '>
								<option value=''>Pilih Tahun</option>";
									...
									while($r=mysql_fetch_array($qsbdth)){
										echo"<option value='$r[tahun]'"; if($thKKWKCC==$r['tahun']){echo"selected";} echo">$r[tahun]</option>";
									}
					echo"</select>
					</div>";
				...
				echo"
					<div class='form-group  col-lg-3'>
						<input type='submit' value='Pilih' class='btn btn-primary'>";
							echo" <a target='_blank' href='print/print_summary_kkwk_cc_pdf.php?id=".ec($_POST['bulan'])."-".ec($thKKWKCC)."&cc=".ec($_POST['cc'])."' class='btn btn-danger'><i class='fa fa-file-pdf-o'></i> PDF</a>
								<a target='_blank' href='print/print_summary_kkwk_cc_excel.php?id=".ec($_POST['bulan'])."-".ec($thKKWKCC)."&cc=".ec($_POST['cc'])."' class='btn btn-success'><i class='fa fa-file-excel-o'></i> EXCEL</a>";
				echo"</div>
				</div>
			</form>";
		?>
					...
					<?php
						...
						if($_POST['opt']=="cari"){
							...
							$query = mysql_query("SELECT DISTINCT	pencapaian.nik,
																	...
																	AND DATE_FORMAT(tgl_aktifitas,'%c %Y')='$month $year'
																	ORDER BY pencapaian.nik
																	");
							timeline($_SESSION['nik'],"cari","pecarian Summary KKWK per CostCenter berdasrkan CostCenter : $_POST[cc] bulan : ".bulan($month)." Tahun: $year ");
						}
