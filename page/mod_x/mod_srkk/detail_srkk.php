...
?>
			...
			<div class="panel panel-inverse">
			    <div class="panel-heading">
			        ...
			        <h4 class="panel-title">Sasaran Rencana Kerja Karyawan - <?=name($getNik)?> Tahun <?=$tahun_aktif?></h4>
			    </div>
			    <div class="panel-body">
			<?php
			
			if(!isset($_GET['opt'])){
				// echo'<a href="?page=detail_srkk&nik='.ec($getNik).'&lvl='.$_GET['lvl'].'&opt=tambah" class="btn btn-sm btn-primary"><i class="fa fa-plus"></i>  Input Target Bulanan</a><p></p>';
				if($_GET['lvl']!=4){
					echo"<a href='page/mod_srkk/aksi_srkk_karyawan.php?tahun=$tahun_aktif&id=".ec($duser['id'])."&unit=".ec($duser['dept'])."&nik=".ec($getNik)."&opt=tambah' class='btn btn-sm btn-success'><i class='fa fa-refresh'></i>  Generate</a><p></p>";
				}else{
					echo"<a href='page/mod_srkk/aksi_srkk_kepala2.php?tahun=$tahun_aktif&id=".ec($duser['id'])."&unit=".ec($duser['dept'])."&nik=".ec($getNik)."&opt=tambah' class='btn btn-sm btn-success'><i class='fa fa-refresh'></i>  Generate</a><p></p>";
				}
			}
							...
							if(isset($_GET['opt'])){
								while($r=mysql_fetch_array($query)){
									...
									<tr>
										...
										for($b=1;$b<=12;$b++){
											$bulanan	= mysql_fetch_array(mysql_query("SELECT * FROM srkk_bulanan WHERE bulan='$b' AND id_srkk='$r[id_srkk]' "));
											echo"<td>
							...
							if(isset($_GET['opt'])){
								while($r=mysql_fetch_array($query)){
									...
									<tr>
										...
										<td>$target[target] $target[satuan]</td>";
										for($b=1;$b<=12;$b++){
											$qId1	= mysql_query("SELECT DISTINCT id FROM wbs WHERE pic='$r[nik]' AND parentId='$r[id_gca]' AND tahun='$tahun_aktif'");
											while($d1=mysql_fetch_array($qId1)){
												$durasi1	= mysql_fetch_array(mysql_query("SELECT count(*) as jum FROM srkk_jam WHERE parentId='$r[id_gca]' AND nik='$r[nik]' AND bulan='$b' AND tahun='$tahun_aktif'"));
												$sd1		= mysql_fetch_array(mysql_query("SELECT count(*) as jum FROM srkk_jam WHERE parentId='$r[id_gca]' AND nik='$r[nik]' AND tahun='$tahun_aktif'"));
												$qId2	= mysql_query("SELECT DISTINCT id FROM wbs WHERE pic='$r[nik]' AND parentId='$d1[id]' AND tahun='$tahun_aktif'");
												while($d2=mysql_fetch_array($qId2)){
													$durasi2	= mysql_fetch_array(mysql_query("SELECT count(*) as jum FROM srkk_jam WHERE parentId='$d1[id]' AND nik='$r[nik]' AND bulan='$b' AND tahun='$tahun_aktif'"));
													$sd2		= mysql_fetch_array(mysql_query("SELECT count(*) as jum FROM srkk_jam WHERE parentId='$d1[id]' AND nik='$r[nik]' AND tahun='$tahun_aktif'"));
													// $qId3	= mysql_query("SELECT DISTINCT id FROM wbs WHERE pic='$r[nik]' AND parentId='$d1[id]'");
													// while($d3=mysql_fetch_array($qId3)){
														// $durasi3	= mysql_fetch_array(mysql_query("SELECT count(*) as jum FROM srkk_jam WHERE parentId='$d2[id_gca]' AND nik='$r[nik]' AND bulan='$b' AND tahun='$tahun_aktif'"));
														// $sd3		= mysql_fetch_array(mysql_query("SELECT count(*) as jum FROM srkk_jam WHERE parentId='$d2[id_gca]' AND nik='$r[nik]' AND tahun='$tahun_aktif'"));
														// $qId4	= mysql_query("SELECT id FROM wbs WHERE pic='$r[nik]' AND parentId='$d1[id]'");
														// while($d4=mysql_fetch_array($qId4)){
															// $durasi4	= mysql_fetch_array(mysql_query("SELECT count(*) as jum FROM waktu_kerja INNER JOIN wbs ON wbs.id = waktu_kerja.id_gca WHERE date_format(tgl_kerja, '%c %Y' ) = '$b $tahun_aktif' AND nik='$r[nik]' AND wbs.parentId='$d3[id]'"));
															// $qId5	= mysql_query("SELECT id FROM wbs WHERE pic='$r[nik]' AND parentId='$d1[id]'");
															// while($d5=mysql_fetch_array($qId5)){
																// $durasi5	= mysql_fetch_array(mysql_query("SELECT count(*) as jum FROM waktu_kerja INNER JOIN wbs ON wbs.id = waktu_kerja.id_gca WHERE date_format(tgl_kerja, '%c %Y' ) = '$b $tahun_aktif' AND nik='$r[nik]' AND wbs.parentId='$d4[id]'"));
																// $qId6	= mysql_query("SELECT id FROM wbs WHERE pic='$r[nik]' AND parentId='$d1[id]'");
																// while($d6=mysql_fetch_array($qId6)){
																	// $durasi6	= mysql_fetch_array(mysql_query("SELECT count(*) as jum FROM waktu_kerja INNER JOIN wbs ON wbs.id = waktu_kerja.id_gca WHERE date_format(tgl_kerja, '%c %Y' ) = '$b $tahun_aktif' AND nik='$r[nik]' AND wbs.parentId='$d5[id]'"));
																	// $qId7	= mysql_query("SELECT id FROM wbs WHERE pic='$r[nik]' AND parentId='$d1[id]'");
																	// while($d7=mysql_fetch_array($qId7)){
																		// $durasi7	= mysql_fetch_array(mysql_query("SELECT count(*) as jum FROM waktu_kerja INNER JOIN wbs ON wbs.id = waktu_kerja.id_gca WHERE date_format(tgl_kerja, '%c %Y' ) = '$b $tahun_aktif' AND nik='$r[nik]' AND wbs.parentId='$d6[id]'"));
