		...
		var x = window.open("lookup/cc3.php", "cc", "toolbar=no,menubar=no,location=center,scrollbars=yes,width=800,height=500");
			...
			<div class="panel panel-inverse">
			    <div class="panel-heading">
			        ...
			        <h4 class="panel-title">Sasaran Rencana Kerja Karyawan - Tahun <?=$tahun_aktif?></h4>
			    </div>
								...
								<select name='unit' class='form-control'>
									<option value=''>Pilih Unit</option>";
										...
										while($unit=mysql_fetch_array($qunit)){
											echo"<option value='".ec($unit['CostCenter'])."' ";if($unit['CostCenter']==dc($_GET['unit'])){echo"selected";} echo">$unit[uraian]</option>";
										}
							echo"</select>
							</div>
							<div class='form-group  col-lg-3'>
				...
				<table id="example1" class="table table-striped table-bordered table-hover nowrap mytable" width="100%">
					...
					<tbody>
						<?php
							...
							if($_SESSION['level']==1){
								if(isset($_POST['opt'])){
									$where2 = "WHERE m_karyawan.dept='$getUnit' AND m_karyawan.status='0'";
								}
							}else{
								if($_SESSION['level']==5){
									$where ="WHERE m_karyawan.dept='$_SESSION[cc]' AND m_karyawan.regno='$_SESSION[nik]' AND m_karyawan.status='0'";
								}elseif($_SESSION['level']==4){
									$where ="WHERE m_karyawan.dept='$_SESSION[cc]' AND m_karyawan.status='0' ";
								}else{
									$where ="WHERE m_karyawan.status='0'";
								}
							}
							...
							while($r=mysql_fetch_array($query)){
								$cek = mysql_num_rows(mysql_query("SELECT nik FROM srkk WHERE nik='$r[regno]'"));
								if($cek < 1){
									$disabled ="disabled";
								}else{
									$disabled ="";
								...
								echo"
									<tr>
										...
										<td>$r[uraian]</td>
										<td>
											<a href='?page=detail_srkk&nik=".ec($r['regno'])."&lvl=$r[level]' class='btn btn-sm btn-primary' ><i class='fa fa-file'></i>  SRKK</a>
											<a target='_blank' href='print/print_srkk.php?nik=".ec($r['regno'])."&lvl=$r[level]' class='btn btn-sm btn-primary' $disabled><i class='fa fa-print'></i>  Cetak</a>
										</td>
									</tr>";
								$no++;
							}
