		...
		echo"<form method='POST' action='page.php?page=summary_kkwk' id='formku'>
				...
				echo"
					<div class='form-group  col-lg-3'>
						...
						if($_SESSION['level']==5){
							echo" <a target='_blank' href='print/print_summary_kkwk.php?id=".ec($_POST['bulan'])."-".ec($_POST['tahun'])."&unit=".ec($_POST['unit'])."&nik=".ec($_SESSION['nik'])."' class='btn btn-danger'><i class='fa fa-file-pdf-o'></i> PDF</a>
								<a target='_blank' href='print/print_summary_kkwk_excel.php?id=".ec($_POST['bulan'])."-".ec($_POST['tahun'])."&unit=".ec($_POST['unit'])."&nik=".ec($_SESSION['nik'])."' class='btn btn-success'><i class='fa fa-file-excel-o'></i> EXCEL</a>";
						}elseif($_SESSION['level']==4){
							echo" <a target='_blank' href='print/print_summary_kkwk.php?id=".ec($_POST['bulan'])."-".ec($_POST['tahun'])."&unit=".ec($_SESSION['cc'])."' class='btn btn-danger'><i class='fa fa-file-pdf-o'></i> PDF</a>
								<a target='_blank' href='print/print_summary_kkwk_excel.php?id=".ec($_POST['bulan'])."-".ec($_POST['tahun'])."&unit=".ec($_SESSION['cc'])."' class='btn btn-success'><i class='fa fa-file-excel-o'></i> EXCEL</a>";
						}else{
							echo" <a target='_blank' href='print/print_summary_kkwk.php?id=".ec($_POST['bulan'])."-".ec($_POST['tahun'])."&unit=".ec($_POST['unit'])."' class='btn btn-danger'><i class='fa fa-file-pdf-o'></i> PDF</a>
								<a target='_blank' href='print/print_summary_kkwk_excel.php?id=".ec($_POST['bulan'])."-".ec($_POST['tahun'])."&unit=".ec($_POST['unit'])."' class='btn btn-success'><i class='fa fa-file-excel-o'></i> EXCEL</a>";
						}
			...
			<table width="100%" border="1" cellpadding="3" style="color:#000000">
				...
				<tbody>
					<?php
						...
						if($_POST['opt']=="cari"){
							if(!empty($_POST['unit'])){
								$unit="AND m_karyawan.dept='$_POST[unit]'";
							}
							if($_SESSION['level']==4){
								$query = mysql_query("SELECT m_karyawan.regno as nik,
																...
																WHERE `user`.`level`!='2' 
																AND `user`.`level`!='3' 
																AND m_karyawan.dept='$_SESSION[cc]' 
							...
							echo"
								<tr>
									...
									for ($d=1;$d<=$endDate;$d++) { 
											...
											echo "<td width='20px' align='center' bgcolor='$bgcolor'><span style=\"color:$fontColor\"><a href='#modal-message' class='edit-record ' data-id='$d-$_POST[bulan]-$_POST[tahun]-$r[cc]-$r[nik]' data-toggle='modal'>".$jumlah_jam."</a></span></td>"; 
										}
