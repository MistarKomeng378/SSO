				...
				<div class="col-md-3 col-sm-6">
					<div class="widget widget-stats bg-blue">
						...
						<div class="stats-info">
							...
							<?php
								$bulanLalu = date("m") - 1;
								$nilaiMSKK = mysql_fetch_array(mysql_query("SELECT SUM(bxs) AS nilai FROM mskk WHERE nik='$_SESSION[nik]' AND bulan='$bulanLalu' AND tahun='$tahun_aktif' "));
								echo"<p>".desimal($nilaiMSKK['nilai'])."</p>";
							?>
						</div>
						<div class="stats-link">
							<a href='#modal-mskk' class='show-mskk' data-id='<?=$bulanLalu."-".$tahun_aktif."-".$_SESSION['cc']."-".$_SESSION['nik']?>' data-toggle='modal'>View Detail <i class="fa fa-arrow-circle-o-right"></i></a>
						</div>
					</div>
				</div>
				<div class="col-md-3 col-sm-6">
			...
			<table class="table table-valign-middle m-b-0">
				...
				<?php
					...
					foreach ($period as $dt) {
						...
						$jml_wk = mysql_fetch_array(mysql_query("SELECT SUM(total_jam) as jum_jam FROM waktu_kerja2 WHERE nik='$_SESSION[nik]' AND bulan='$bulan' AND tahun='$tahun'"));
						
						if($jml_wk['jum_jam'] != $jumJamKerja){
							$fontColor="red"; 
						}else{
							$fontColor="black"; 
						}
						echo"
							<tr>
								...
								<td>$x</td>
								<td>$jumJamKerja</td>
								<td><a href='#modal-message' class='gca-detail' data-id='".ec($_SESSION['nik'])."-".ec($b)."-".ec($year)."-".ec($_SESSION['cc'])."' data-toggle='modal'><span style=\"color:$fontColor\"><b>$jml_wk[jum_jam]</b></span></a></td>
							</tr>";
						$b++;	
					}
				?>
...
<div class="row">
		...
		<div class="panel panel-inverse">
			...
			<div class="panel-body">
							...
							<select name='tahun' class='form-control '>
								<option value=''>Pilih Tahun</option>";
									...
									while($r=mysql_fetch_array($qsbdth)){
										echo"<option value='$r[tahun]'"; if($thDSB==$r['tahun']){echo"selected";} echo">$r[tahun]</option>";
									}
						echo"</select>
						</div>
				...
				";
					if(isset($_POST['cari'])){
					$data=mysql_query("SELECT	srko.kpm,
										progress_srko.id_srko,
										progress_srko.target,
										progress_srko.pencapaian
										FROM
										progress_srko
										INNER JOIN srko ON srko.id_srko = progress_srko.id_srko 
										WHERE progress_srko.bulan='$getBulan' 
										AND progress_srko.tahun='$getTahun' 
										AND progress_srko.cc='$getCC' ORDER BY id_srko");
					$data2=mysql_query("SELECT	srko.kpm,
										progress_srko.id_srko,
										progress_srko.target,
										progress_srko.pencapaian
										FROM
				...
				<?php
					if(isset($_POST['cari'])){
						// if($getInsert==1){
							// echo"<a href='?page=ket_progress&unit=".ec($getCC)."-".ec($getBulan)."' class='btn btn-primary'><i class='fa fa-plus'></i> Input Keterangan</a>";
						// }
						...
						<table class='table table-bordered'>
							<thead>
								<tr align='center' bgcolor='#b3d9ff'>
									...
									<td colspan='3'><b>Target Bulanan</b></td>
									<td colspan='3'><b>S/d Bulan Berjalan</b></td>
								</tr>
								<tr bgcolor='#b3d9ff'>
									<td><b>Target</b></td>
									<td><b>Realisasi</b></td>
							...
							<tbody>";
							$query = mysql_query("SELECT DISTINCT id_srko,rencana_kerja,bobot,target,satuan,hasil_akhir FROM srko WHERE CostCenter='$getCC' AND tahun='$getTahun' order by id_srko");
						while($r=mysql_fetch_array($query)){
