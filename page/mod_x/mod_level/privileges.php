...
<div class="box box-primary">
	<form role="form" method="POST" action="page/mod_level/query_privil.php" id="formku">
		<div class="box-body">
		<?php
		...
		?>
			...
			<table class="table table-bordered table-striped table-hover" >
				...
				<tbody>
					<?php
						$no=1;
						$level 	= mysql_real_escape_string($_GET['set']);
						echo"<input type='hidden' name='level' value='$level[level]'>";
						$query 	= mysql_query("SELECT * FROM m_menu WHERE status='1'");
						while($r=mysql_fetch_array($query)){
							$role = mysql_num_rows(mysql_query("SELECT DISTINCT * FROM role_menu_temp WHERE id_menu='$r[id_menu]' AND level='$level[level]' "));
							echo"
								<tr>
									<td>$no</td>
									<td>$r[id_menu]</td>
									...
									while($p=mysql_fetch_array($getPer)){
										$role_per=mysql_num_rows(mysql_query("SELECT DISTINCT * FROM role_permission_temp WHERE id_permission='$p[id_permission]' AND id_menu='$r[id_menu]' AND level='$level[level]'"));
