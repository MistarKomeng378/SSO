	...
	<div class="panel panel-inverse">
		...
		if($_GET['opt']=="perspektif"){
			...
			if($_GET['act']=="tambah"){
				...
				$edit		= mysql_fetch_array(mysql_query("SELECT * FROM kpku_perspektif WHERE id_perspektif='$id_baru' "));
				$tahun		= $edit['tahun'];
				...
				<form method='POST' action='page/mod_kpi/query_kpku_kpi.php?opt=perspektif&act=$_GET[act]' id='formku'>
					<div class='form-group  col-lg-12 row'>
						<div class='form-group  col-lg-2'>
							<input type='text' name='id_perspektif' class='form-control required' value='$id_baru' placeholder='Id Prespektif'>
						</div>
			...
			if($_GET['act']=="tambah"){
				...
				$id_baru 	= mysql_real_escape_string(dc($_GET['id']));
				$edit		= mysql_fetch_array(mysql_query("SELECT * FROM kpku_kpi WHERE id_perspektif='$getid' AND id_kpi='$id_baru' "));
				$tahun		= $edit['tahun'];
				...
				<form method='POST' action='page/mod_kpi/query_kpku_kpi.php?opt=kpi_kpku&act=$_GET[act]' id='formku'>
					<div class='form-group  col-lg-6'>
						<div class='form-group  col-lg-4'>
							<label>Id KPI</label>
							<input type='text' name='id_kpi' class='form-control required' value='$id_baru' placeholder='Id KPI'>
			...
			if($_GET['act']=="tambah"){
				$id 		= mysql_fetch_array(mysql_query("SELECT MAX(id_kpi) as id FROM kpku_kpi WHERE id_perspektif=''"));
				$id_baru	= $id['id']+1;
				$tahun		= date("Y");
				$id_srko	= "";
				$kpi		= "";
					...
					echo"<tr>
							...
							<td>";
								if($getEdit==1){
									...
									echo" <a href='?page=form_kpku_kpi&opt=kpi_kpku2&act=tambah&delete=".ec($r['id_kpi'])."' class='btn btn-xs btn-danger' title='Hapus Prespektif' ><i class='fa fa-trash'></i></a>";
								}
						echo"</td>
						</tr>";
					$no++;
