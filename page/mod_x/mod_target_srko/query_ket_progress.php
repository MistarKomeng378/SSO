...
if($_GET['opt']=="analisa"){
	...
	if($_GET['act']=="input"){
		mysql_query("INSERT INTO `ket_progress_srko` SET 	`id_ket`			='$id_ket',
												...
												");
		header("location:../../page.php?page=ket_progress&unit=".ec($id_srko)."-".ec($cc)."-".ec($bulan)."-".ec($id_ket)."&act=".$_GET['act']."&opt=hasil");
		...
		header("location:../../page.php?page=ket_progress&unit=".ec($id_srko)."-".ec($cc)."-".ec($bulan)."&opt=view");
	}
	...
	header("location:../../page.php?page=ket_progress&unit=".ec($getID)."-".ec($getCC)."-".ec($newBulan)."&opt=view");
}
?>