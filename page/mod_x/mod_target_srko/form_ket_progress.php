...
<script type="text/javascript" src="ckeditor/ckeditor.js"></script >
			...
			<div class="panel panel-inverse">
			    <div class="panel-heading">
			        ...
			        <h4 class="panel-title">From Keterangan Progress SRKO Bulan <?=bulan($getBulan)?> Tahun <?=$tahun_aktif?></h4>
			    </div>
	...
	if($_GET['opt']=="analisa"){
		if($_GET['act']=="edit"){
			$analisa = mysql_fetch_array(mysql_query("SELECT * FROM ket_progress_srko WHERE id_ket='$getKet' AND id_srko='$getID' AND cc='$getCC' AND bulan='$getBulan' AND tahun='$tahun_aktif'"));
...
<?php
			...
			if($_GET['act']=="input"){
				$hasil 	= mysql_fetch_array(mysql_query("SELECT CONCAT( `what`,'. ', `who`,'. ', `when`,'. ', `where`,'. ', `why`,'. ', `how`,'. ', `how_much`,'. ', `time`,'. ', `place`,'. ', `organization`) as hasil FROM ket_progress_srko WHERE id_ket='$getKet' AND id_srko='$getID' AND cc='$getCC' AND bulan='$getBulan' AND tahun='$tahun_aktif'"));
				$isi	= $hasil['hasil'];
			}elseif($_GET['act']=="edit"){				
				$hasil 	= mysql_fetch_array(mysql_query("SELECT id_ket,hasil_analisa FROM ket_progress_srko WHERE id_ket='$getKet' AND id_srko='$getID' AND cc='$getCC' AND bulan='$getBulan' AND tahun='$tahun_aktif'"));
...
?>
	<h5>Form isian keterangan srko : <b><?=$getID?></b>. <?=$srko['rencana_kerja']?></h5><hr>
	<a href="<?php echo"?page=ket_progress&unit=".ec($getID)."-".ec($getCC)."-".ec($getBulan)."&act=input&opt=analisa";?>" class='btn btn-primary'><i class='fa fa-plus'></i> Input Keterangan</a>
	<?php echo"<a href='lookup/ket_progress2.php?id=".ec($getID)."-".ec($getCC)."-".ec($newBulan)."-".ec($tahun_aktif)."' class='btn btn-success popup'  title='Lihat Keterangan'><i class='fa fa-table'></i> Bulan sebelumnya</a>";?>
	<!--<a href="<?php //echo"page/mod_target_srko/query_ket_progress.php?id=".ec($getID)."-".ec($getCC)."-".ec($getBulan)."-".ec($tahun_aktif)."&opt=recycle";?>" class='btn btn-success'><i class='fa fa-refresh'></i> Input Dari bulan sebelumnya</a>-->
	<hr>
	<table class="table table-bordered" id="example1">
		...
		<tbody>
			<?php
				$no=1;
				$qket = mysql_query("SELECT id_ket,id_srko,hasil_analisa,ket_progress FROM ket_progress_srko WHERE id_srko='$getID' AND cc='$getCC' AND bulan='$getBulan' AND tahun='$tahun_aktif'");
				while($r=mysql_fetch_array($qket)){
					...
					<tr>
						...
						<td>$str...</td>
						<td>
							<a href='?page=ket_progress&unit=".ec($r['id_srko'])."-".ec($getCC)."-".ec($getBulan)."-".ec($r['id_ket'])."&act=edit&opt=analisa' class='btn btn-xs btn-success' title='Edit Analisa Masalah' ><i class='fa fa-pencil'></i></a> 
							<a href='?page=ket_progress&unit=".ec($r['id_srko'])."-".ec($getCC)."-".ec($getBulan)."-".ec($r['id_ket'])."&act=edit&opt=hasil' class='btn btn-xs btn-success' title='Edit Resume' ><i class='fa fa-pencil'></i></a> 
							<a href='?page=ket_progress&unit=".ec($r['id_srko'])."-".ec($getCC)."-".ec($getBulan)."-".ec($r['id_ket'])."&delete=1&opt=view' class='btn btn-xs btn-danger' onClick=\"return confirm('Yakin Hapus Data?')\" title='Delete Keterangan' ><i class='fa fa-trash'></i></a>
						</td>
					</tr>
					";
				$no++;
