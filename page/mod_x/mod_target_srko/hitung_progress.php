...
///////////////////////////////////////////////////////////
	if($d['jenisGCA']==1){
		...
		$wk = mysql_query("select id,parentId,tahun,
								ifnull((SELECT count(bulan) FROM waktu_kerja2 WHERE id_gca='$d[id]' AND tahun='$getTahun'),'') as bln,
								ifnull((SELECT total_jam FROM waktu_kerja2 WHERE id_gca='$d[id]' AND bulan='$getBulan' AND tahun='$getTahun'),'') as total_jam,
								ifnull((select SUM(total_jam) FROM waktu_kerja2 WHERE id_gca='$d[id]' AND tahun='$getTahun'),'') as jmlJam
								from wbs 
...
$wbs 	= mysql_fetch_array(mysql_query("SELECT id,parentId,aktivitas,durasi,cc_id,level,tahun,jenisGCA,jenisAktf FROM wbs WHERE id='$p[parentId]' "));						
