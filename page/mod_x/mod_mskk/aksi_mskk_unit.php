	...
	while($r=mysql_fetch_array($queryKaryawan)){
		if($r['level']!=4){
			...
			while($r2=mysql_fetch_array($queryMSKK)){
				...
				// echo"==========================================================================================================<br>";
				$mskkb			= mysql_query("SELECT * FROM mskk_bulanan WHERE id_srkk='$r2[id_gca]' AND bulan='$getBulan' AND tahun='$getTahun' AND nik='$r[nik]' ");
				$totJam	 		= mysql_fetch_array(mysql_query("SELECT SUM(hari) as hari FROM mskk_bulanan WHERE nik='$r[nik]' AND id_srkk='$r2[id_gca]' AND bulan='$getBulan' AND tahun='$getTahun' "));				
				$nilai	 		= mysql_fetch_array(mysql_query("SELECT nilai FROM srkk WHERE nik='$r[nik]' AND id_gca='$r2[id_gca]' AND tahun='$getTahun' "));				
			...
			$queryMSKK2 = mysql_query("SELECT * FROM mskk WHERE bulan='$getBulan' AND nik='$r[nik]' AND tahun='$getTahun'");
			while($ms=mysql_fetch_array($queryMSKK2)){
				$bobotA		 	= mysql_fetch_array(mysql_query("SELECT srkk.bobot as bobot FROM srkk INNER JOIN mskk ON mskk.nik = srkk.nik AND mskk.id_gca = srkk.id_gca WHERE mskk.nik='$r[nik]' AND mskk.tahun='$getTahun' AND mskk.bulan='$getBulan' AND mskk.id_gca='$ms[id_gca]'"));				
				$sumbbt		 	= mysql_fetch_array(mysql_query("SELECT SUM(srkk.bobot) as bobot FROM mskk INNER JOIN srkk ON srkk.id_gca = mskk.id_gca WHERE srkk.nik='$r[nik]' AND mskk.nik='$r[nik]' AND srkk.tahun='$getTahun' AND mskk.bulan='$getBulan' "));				
				
				$bobot = ($bobotA['bobot']/$sumbbt['bobot'])*75;
				$bxs	= $bobot * $ms['score'];
				mysql_query("UPDATE mskk SET bobotA='$bobotA[bobot]',bobot='$bobot',bxs='$bxs' WHERE  nik='$r[nik]' AND id_gca='$ms[id_gca]' AND bulan='$getBulan' AND tahun='$getTahun'");
			}
		}		
	}
