...
include"../../config/fungsi_rupiah.php";
	...
	while($r=mysql_fetch_array($queryKaryawan)){
		if($r['level']!=4){
			...
			$queryMSKK2 = mysql_query("SELECT * FROM mskk WHERE bulan='$getBulan' AND nik='$getNik' AND tahun='$getTahun'");
			while($ms=mysql_fetch_array($queryMSKK2)){
				$bobotA		 	= mysql_fetch_array(mysql_query("SELECT srkk.bobot as bobot FROM srkk INNER JOIN mskk ON mskk.nik = srkk.nik AND mskk.id_gca = srkk.id_gca WHERE mskk.nik='$getNik' AND mskk.tahun='$getTahun' AND mskk.bulan='$getBulan' AND mskk.id_gca='$ms[id_gca]'"));				
				$sumbbt		 	= mysql_fetch_array(mysql_query("SELECT SUM(srkk.bobot) as bobot FROM mskk INNER JOIN srkk ON srkk.id_gca = mskk.id_gca WHERE srkk.nik='$getNik' AND mskk.nik='$getNik' AND srkk.tahun='$getTahun' AND mskk.bulan='$getBulan' "));				
	...
	header('Location: ../../page.php?page=detail_mskk&id='.ec($getNik).'-'.ec($getBulan).'-'.ec($getTahun).'&lvl='.$getLvl.'&succes=1');
?>