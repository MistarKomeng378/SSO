...
$total_durasi 	= $durasi - $count +1;
$kpi			= mysql_fetch_array(mysql_query("SELECT kode_kpi FROM wbs WHERE id='$parentId' "));
/////////////////proses simpan////////////////////////////
	mysql_query("DELETE FROM waktu_kerja2 WHERE id_gca='$parentId'");
	$id_baru	= mysql_fetch_array(mysql_query("SELECT max(id) as id FROM wbs  WHERE cc='$cc' ORDER by parentId DESC"));
	...
	for($i=0; $i<$jum1; $i++){
		...
		if($total_durasi==0){
			...
			$tdurasi=$total_durasi;
		}
		// echo "$tdurasi $durasi <br>";
		// echo "-->$id_lama <br>";
		// $cekPic	= mysql_fetch_array(mysql_query("SELECT pic FROM wbs WHERE id='$id'"));
			if($pic_asli!=$pic){
				if($jmlpic > 0){
					$idBaru = $id_lama;
				}else{
		...
		if($_GET['opt']=="edit"){
			mysql_query("DELETE FROM waktu_kerja2 WHERE `id_gca`='$idBaru' AND `tahun`='$tahun_aktif' ");
		}else{
			mysql_query("DELETE FROM waktu_kerja2 WHERE `id_gca`='$id' AND `tahun`='$tahun_aktif' ");
		}
		...
		foreach ($period as $dt) {
			...
			if($jam !==""){
				...
				for ($tgl=0;$tgl<$lastDay;$tgl++){				
					...
					if(date("D",mktime (0,0,0,$month,$d,$year)) == "Sun") {
						$time="";
						// echo"<span style=\"color:red\"><b>$tgl_kerja ($time)/ </b></span>";
					}elseif(date("D",mktime (0,0,0,$month,$d,$year)) == "Sat") {
						$time="";
						// echo"<span style=\"color:blue\"><b>$tgl_kerja ($time)/ </b></span>";
					}else{
						...
						if($delLibur['tanggal']==$tgl_baru){
							...
							if($status == 0){
								if($_GET['opt']=="edit"){
									mysql_query("UPDATE waktu_kerja2 SET  `$d`='$time' WHERE `nik`='$pic' AND `id_gca`='$idBaru' AND bulan='$bln' AND tahun='$year' ");
								}else{
									mysql_query("UPDATE waktu_kerja2 SET  `$d`='$time' WHERE `nik`='$pic' AND `id_gca`='$id' AND bulan='$bln' AND tahun='$year' ");
								}
								// echo"$tgl_baru $status ($time)/<br> ";
							}
				...
				if($_GET['opt']=="edit"){
					mysql_query("UPDATE waktu_kerja2 SET total_hari='$jumlahHari' WHERE nik='$pic' AND bulan='$month' AND tahun='$year' AND id_gca='$idBaru'");
				}else{
			...
			// echo"<br>";
			if($_GET['opt']=="edit"){
				$update_total_jam = mysql_query("SELECT distinct id_gca,nik,bulan FROM waktu_kerja2 WHERE id_gca='$idBaru'");
			}else{
			...
			while($rj=mysql_fetch_array($update_total_jam)){	
				$durasi_wk	= mysql_fetch_array(mysql_query("SELECT SUM(`1`)+SUM(`2`)+SUM(`3`)+SUM(`4`)+SUM(`5`)+SUM(`6`)+SUM(`7`)+SUM(`8`)+SUM(`9`)+SUM(`10`)+
							...
							SUM(`31`)+SUM(`32`) as jum FROM waktu_kerja2 WHERE id_gca='$rj[id_gca]' AND bulan='$rj[bulan]' AND nik='$rj[nik]' "));
...
<div id="content" class="content">
	...
	<ol class="breadcrumb pull-right">
		<li><a href="javascript:;">GCA</a></li>
		<li><a href="javascript:;">GCA Load</a></li>
		<li><a href="javascript:;">Validasi GCA</a></li>
	</ol>
	<!-- end breadcrumb -->
	<!-- begin page-header -->
	...
	<div class="panel panel-inverse">
		...
		<button type="submit" name="simpan" value="simpan" class="btn btn-primary">Selesai</button>
		<hr>
