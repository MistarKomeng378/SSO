...
?>	
		<h1 class="page-header"> Satuan Penilaian Sasaran Kerja
			...
			<div class="panel panel-inverse">
			    <div class="panel-heading">
			        ...
			        <h4 class="panel-title">Satuan Penilaian Sasaran Kerja <?=$getTahun?></h4>
			    </div>
			...
			if(isset($_REQUEST['delete'])){
				...
				mysql_query("DELETE FROM satuan WHERE id='$getId'");
		...
		<table border="0" width="100%">
			<tr>
				<td>
					<div class='col-lg-8'>
					<a href='?page=form_satuan&opt=tambah' class='btn btn-primary'><i class='fa fa-plus'></i> Input Satuan</a>
			</div>
				</td>
			</tr>
		</table>
		...
		<table id="example1" class="table table-bordered table-striped table-hover" width="70%">
				...
				<tbody>
					<?php
						$query = mysql_query("SELECT * FROM satuan order by satuan ASC ");
						$i=1;
						while($r=mysql_fetch_array($query)){
							echo"
								<tr>
									...
									//=====================
									if($getEdit==1){
										echo"<a href='?page=form_satuan&opt=edit&id=".ec($r['id'])."' class='btn btn-xs btn-primary' title='Edit'><i class='fa fa-pencil'></i></a> ";
									}if($getDelete==1){
										echo"<a href='?page=satuan&delete=".ec($r['id'])."' class='btn btn-xs btn-danger' title='Delete' onClick=\"return confirm('Yakin Hapus Data?')\"><i class='fa fa-trash'></i></a>";
									}
									echo"
									</td>
								</tr>
