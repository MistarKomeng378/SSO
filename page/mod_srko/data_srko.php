    ...
    .mytable th{
        background-color: #ccd9ff
							...
							<div class="form-group  col-lg-12 row">
								...
								<div class="form-group  col-lg-8">
									...
									<select name="unit" class="form-control">
										...
										<?php
											$qunit = mysql_query("SELECT * FROM mskko WHERE id !='1.6' AND id!='4' AND id!='1.5' AND id!='5' AND id!='6'order by id");
			...
			if(isset($_REQUEST['delete'])){
				...
				mysql_query("DELETE FROM srko WHERE parent_srko='$getId'");
				mysql_query("DELETE FROM target_srko WHERE parent_srko='$getId'");
				mysql_query("DELETE FROM target_srko_detile WHERE id_srko='$getId'");
				mysql_query("DELETE FROM progress_srko WHERE parent_srko='$getId'");
				mysql_query("DELETE FROM progress_srko_detile WHERE id_srko='$getId'");
				mysql_query("DELETE FROM wbs WHERE id_srko='$getId'");
				mysql_query("DELETE FROM pic WHERE id_srk='$getId'");
				mysql_query("DELETE FROM integrasi WHERE id_srk='$getId'");
					...
					<?php
						...
						if(isset($_GET['unit'])){
							$query = mysql_query("SELECT DISTINCT srko.id_srko,
													...
													AND srko.parent_srko='' ORDER BY srko.id_srko, srko.id_kpi ASC
													");
						}else{
							$query = mysql_query("SELECT DISTINCT srko.id_srko,
													kpi.kpi,
													...
													AND parent_srko='' ORDER BY srko.id_srko ASC
													");
						}
						...
						while($r=mysql_fetch_array($query)){
							...
							echo"
								<tr>
									...
									<td>$r[rencana_kerja]";
										// $ff = mysql_fetch_array(mysql_query("select * from srko_detile where parent_srko='$r[id_srko]'"));
										...
										while($P=mysql_fetch_array($h)){
											echo"
												<br>$P[rencana_kerja] ";
													if(isset($_GET['unit'])){
														if($getEdit==1){
															echo" < <a href='?page=form_srko&opt=edit&id=".ec($P['id_srko'])."&cc=".ec($getUnit)."' title='Edit'>Edit</a> > &nbsp;"; 
														}if($getDelete==1){
															echo" < <a href='?page=data_srko&unit=".ec($getUnit)."&delete=".ec($P['id_srko'])."' title='Delete' onClick=\"return confirm('Yakin Hapus Data&nbsp; $P[rencana_kerja] ?')\">Hapus</a> >";
														}
													}else{
														if($getEdit==1){
															echo" > <a href='?page=form_srko&opt=edit&id=".ec($P['id_srko'])."'  title='Edit'>Edit</a> &nbsp;";
														}if($getDelete==1){
															echo" > <a href='?page=data_srko&delete=".ec($P['id_srko'])."'  title='Delete' onClick=\"return confirm('Yakin Hapus Data?')\">Hapus</a>";
														}
													}
									...
									<td>";
										if(isset($_GET['unit'])){
											if($getEdit==1){
												...
												echo"<a href='?page=data_srko&unit=".ec($getUnit)."&delete=".ec($r['id_srko'])."' class='btn btn-xs btn-danger' title='Delete' onClick=\"return confirm('Yakin Hapus Data ?')\"><i class='fa fa-trash'></i></a>";
											}
										}else{
											if($getEdit==1){
