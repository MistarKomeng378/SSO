...
<div class="panel panel-inverse">
	<div class="panel-heading">
		...
		<h4 class="panel-title">Key Performance Indicators & Target Bulan <?=bulan($getBulan)?> Tahun <?=$thKPI?></h4>
	</div>
...
<?php
						...
						<select name='tahun' class='form-control '>
								<option value=''>Pilih Tahun</option>";
									...
									while($r=mysql_fetch_array($qsbdth)){
										echo"<option value='$r[tahun]'"; if($thKPI==$r['tahun']){echo"selected";} echo">$r[tahun]</option>";
									}
					echo"</select>
					</div>
		...
		?>
			...
			<table class="table table-bordered table-striped table-hover">
				...
				<?php
				if(isset($_POST['opt'])){
					$query = mysql_query("SELECT * FROM kpku_perspektif WHERE tahun='$getTahun' Order by id_perspektif ASC ");
					...
					echo"<tr>
						...
						echo"<tr>
								...
								<td><a href='?page=chart_kpi&id=".ec($r2['id_kpi'])."-".ec($getTahun)."' target='_blank' class='btn btn-xs btn-warning' title='Chart View' ><i class='fa fa-bar-chart'></i> </a></td>
							</tr>";
						}
					}
				}
			...
			<?php
				if($getInsert==1){
					echo"
						<a href='?page=form_kpku_kpi&opt=kpi_kpku2&tahun=".ec($getTahun)."&act=tambah' class='btn btn-sm btn-primary'><i class='fa fa-plus'></i> Tambah KPI KPKU</a>
					<hr>";
				}
			?>
		...
		<table class="table table-bordered table-striped table-hover">
			...
			<tbody>
				...
				if(isset($_POST['opt'])){
					...
					// $query = mysql_query("SELECT * FROM kpku_kpi WHERE id_perspektif='' ");
					while($r=mysql_fetch_array($query)){
						...
						echo"<tr>
								...
								<td>".desimal3($target['pencapaian'])." %</td>
								<td>".desimal3($nilai)." %</td>
								<td><a href='?page=chart_kpi&id=".ec($r['id_kpi'])."-".ec($getTahun)."-".ec($getBulan)."' target='_blank' class='btn btn-xs btn-warning' title='Chart View' ><i class='fa fa-bar-chart'></i> </a></td>
							</tr>";
					}
				}
				?>
