			...
			<div class="panel panel-inverse">
			    <div class="panel-heading">
			        ...
			        <h4 class="panel-title">Data KPI <?echo date('Y');?></h4>
			    </div>
			...
			<table id="example1" class="table table-bordered table-striped table-hover" >
				...
				<tbody>
					<?php
						...
						$query = mysql_query("SELECT * FROM kpi where tahun='".date('Y')."'ORDER BY tahun,id_kpi");
						...
						while($r=mysql_fetch_array($query)){							
							if($_SESSION['level']==1){
								...
								if($r['status_kpi']==1){
									$status="<a href='#' title='Nonaktifkan User' class='btn-xs btn-primary btn-flat' ><i class='fa fa-check'></i></a>";
								}else{
									$status="<a href='#' title='Aktifkan User' class='btn-xs btn-danger btn-flat' ><i class='fa fa-times'></i></a>";
								}
							}
							...
							echo"
								<tr>
									...
									<td>";
									if($getEdit==1){
										echo"<a href='?page=form_kpi&opt=edit&id=".ec($r['id_kpi'])."&tahun=$r[tahun]' class='btn btn-xs btn-primary' title='Edit'><i class='fa fa-pencil'></i></a> ";
									}if($getDelete==1){
										echo"<a href='?page=katalog_kpi&delete=".ec($r['id_kpi'])."&tahun=$r[tahun]' class='btn btn-xs btn-danger' title='Delete' onClick=\"return confirm('Yakin Hapus Data?')\"><i class='fa fa-trash'></i></a>";
									}
								echo"</td>
								</tr>
							";
