...
?>
			...
			<div class="panel panel-inverse">
			    <div class="panel-heading">
			        ...
			        <h4 class="panel-title">Matrik Sasaran Kerja Karyawan - Tahun <?=$thMSKK?></h4>
			    </div>
			...
			if(isset($_REQUEST['lock'])){
				...
				mysql_query("UPDATE mskk SET `lock`='1' WHERE bulan='$bln' AND tahun='$thMSKK'");
				...
				$bln	= mysql_real_escape_string($_GET['bulan']);
				mysql_query("UPDATE mskk SET `lock`='0' WHERE bulan='$bln'AND tahun='$thMSKK' ");
				echo"<div class='alert alert-danger alert-dismissable'>
                        <i class='fa fa-check'></i>
                        <button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
						...
						<div class='form-group  col-lg-12 row'>
								...
								<select name='tahun' class='form-control '>
										<option value=''>Pilih Tahun</option>";
											...
											while($r=mysql_fetch_array($qsbdth)){
												echo"<option value='".ec($r['tahun'])."'"; if($thMSKK==$r['tahun']){echo"selected";} echo">$r[tahun]</option>";
											}
							echo"</select>
							</div>
							<div class='form-group  col-lg-4'>
								...
								if($_SESSION['level']==1 AND isset($_POST['opt'])){
									if($getInsert==1){
										echo" <a href='page/mod_mskk/aksi_mskk_unit.php?tahun=$thMSKK&id=".ec($idunit['id'])."&unit=".ec($getUnit)."&bulan=".ec($getBulan)."&opt=tambah' class='btn btn btn-success'><i class='fa fa-refresh'></i> Generate Unit</a>";
									}								
								}								
							echo"
							</div>
				...
				<table id="example1" class="table table-striped table-bordered table-hover nowrap mytable" width="100%">
					...
					<tbody>
						<?php
							...
							if($_SESSION['level']==1){
								if(isset($_POST['opt'])){
									$and2 = "AND m_karyawan.dept='$getUnit'";
								}
							}else{
								if($_SESSION['level']==5){
									$and ="AND m_karyawan.dept='$_SESSION[cc]' AND m_karyawan.regno='$_SESSION[nik]'";
									// $where ="WHERE m_karyawan.dept='$_SESSION[cc]' ";
								}elseif($_SESSION['level']==4){
									$and ="AND m_karyawan.dept='$_SESSION[cc]'";
								}else{
									$and ="";
								}
							}
							...
							while($r=mysql_fetch_array($query)){
								$cek = mysql_num_rows(mysql_query("SELECT nik FROM mskk WHERE nik='$r[regno]' AND bulan='$getBulan' AND tahun='$tahun_aktif'"));
								if($cek < 1){
									$disabled ="disabled";
								}else{
									$disabled ="";
								...
								echo"
									<tr>
										...
										<td>$r[uraian]</td>
										<td>
											<a target='_blank' href='?page=detail_mskk&id=".ec($r['regno'])."-".ec($getBulan)."-".ec($thMSKK)."&lvl=$r[level]' class='btn btn-sm btn-primary' ><i class='fa fa-file'></i>  MSKK</a>
											<a target='_blank' href='print/print_mskk.php?id=".ec($r['regno'])."-".ec($getBulan)."-".ec($thMSKK)."&lvl=$r[level]' class='btn btn-sm btn-primary' $disabled><i class='fa fa-print'></i>  Cetak</a>
										</td>
									</tr>";
								}
								$no++;
