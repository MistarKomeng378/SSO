...
<!--------------------------------------------------------------------------------------------------------------------->
		...
		<?php
			...
			if(isset($_REQUEST['delete'])){
				mysql_query("DELETE FROM m_menu WHERE id_menu='$_REQUEST[delete]' ");
				mysql_query("DELETE FROM m_menu WHERE parentId='$_REQUEST[delete]' ");
			...
			if(isset($_REQUEST['subdelete'])){
				mysql_query("DELETE FROM sub_menu WHERE id_sub='$_REQUEST[subdelete]' ");
				echo"<div class='alert alert-danger alert-dismissable'>
                        <i class='fa fa-check'></i>
                        <button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
                        <b>Succes!</b> Data Berhasil Dihapus.
		...
		?>
			<table class="table table-bordered table-striped table-hover" >
				...
				<tbody>
					<?php
						$no=1;
						$query = mysql_query("SELECT * FROM m_menu ORDER BY id_menu, parentId ASC");
								...
								<tr>
									<td>
                                        <div class='btn-group'>
											...
											if($getInsert==1){
												echo"<li><a href='page.php?page=form_submenu&&opt=tambah&id=".ec($r['id_menu'])."'><i class='fa fa-plus'></i> Tambah Submenu</a></li>";
											}if($getEdit==1){
