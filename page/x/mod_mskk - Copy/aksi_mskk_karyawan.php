...
include"../../config/fungsi_rupiah.php";
	...
	while($r=mysql_fetch_array($queryKaryawan)){
		if($r['level']!=4){
			...
			$queryMSKK = mysql_query("SELECT * FROM srkk WHERE  nik='$getNik' ");
			while($r2=mysql_fetch_array($queryMSKK)){
				...
				// echo"==========================================================================================================<br>";
				$mskkb			= mysql_query("SELECT * FROM mskk_bulanan WHERE id_srkk='$r2[id_gca]' AND bulan='$getBulan' AND tahun='$tahun_aktif' AND nik='$getNik' ");
				$totJam	 		= mysql_fetch_array(mysql_query("SELECT SUM(hari) as hari FROM mskk_bulanan WHERE nik='$getNik' AND id_srkk='$r2[id_gca]' AND bulan='$getBulan' AND tahun='$tahun_aktif' "));				
				$nilai	 		= mysql_fetch_array(mysql_query("SELECT nilai FROM srkk WHERE nik='$getNik' AND id_gca='$r2[id_gca]' AND tahun='$tahun_aktif' "));				
				$target 		= mysql_fetch_array(mysql_query("SELECT prosen FROM srkk_bulanan WHERE id_gca='$r2[id_gca]' AND bulan='$getBulan' AND nik='$getNik' "));
				$treal			= 0;
				$bagi 			= 0;
			...
			$queryMSKK2 = mysql_query("SELECT * FROM mskk WHERE bulan='$getBulan' AND nik='$getNik' AND tahun='$tahun_aktif'");
			while($ms=mysql_fetch_array($queryMSKK2)){
				$bobotA		 	= mysql_fetch_array(mysql_query("SELECT srkk.bobot as bobot FROM srkk INNER JOIN mskk ON mskk.nik = srkk.nik AND mskk.id_gca = srkk.id_gca WHERE mskk.nik='$getNik' AND mskk.tahun='$tahun_aktif' AND mskk.bulan='$getBulan' AND mskk.id_gca='$ms[id_gca]'"));				
				$sumbbt		 	= mysql_fetch_array(mysql_query("SELECT SUM(srkk.bobot) as bobot FROM mskk INNER JOIN srkk ON srkk.id_gca = mskk.id_gca WHERE srkk.nik='$getNik' AND mskk.nik='$getNik' AND srkk.tahun='$tahun_aktif' AND mskk.bulan='$getBulan' "));				
				
				$bobot = ($bobotA['bobot']/$sumbbt['bobot'])*75;
				$bxs	= $bobot * $ms['score'];
				
				mysql_query("UPDATE mskk SET bobotA='$bobotA[bobot]',bobot='$bobot',bxs='$bxs' WHERE  nik='$getNik' AND id_gca='$ms[id_gca]' AND bulan='$getBulan' AND tahun='$tahun_aktif'");
			}
		}		
	}
	mysql_query("DELETE FROM mskk WHERE tahun=''");
	header('Location: ../../page.php?page=detail_mskk&id='.ec($getNik).'-'.ec($getBulan).'&lvl='.$getLvl.'&succes=1');
?>