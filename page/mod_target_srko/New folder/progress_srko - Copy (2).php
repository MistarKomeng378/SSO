						...
						while($r=mysql_fetch_array($query)){
							...
							<tr>
								...
								for($i=1;$i<=13;$i++){
									...
									$count = mysql_fetch_array(mysql_query("select COUNT(target) as jumlah from target_srko where parent_srko='$r[id_srko]' AND tahun='$tahun_tsrko' AND bulan='$i'"));
									...
									if($i==13){
										...
										echo"<td width='5%' bgcolor='#ffcccc' align='center'>".desimal3($target['target']/$quan)."</td>";
									}
								}
						echo"</tr>";
								...
								for($row=1;$row<=2;$row++){
									...
									echo"<tr bgcolor='$rowcolor'>
										...
										for($cols=1;$cols<=13;$cols++){
											...
											$prog = mysql_fetch_array(mysql_query("SELECT * FROM progress_srko WHERE bulan='$cols' AND tahun='$tahun_tsrko' AND id_srko='$r[id_srko]'"));
											
											$realis = mysql_fetch_array(mysql_query("SELECT sum(realisasi) as sumreal FROM progress_srko WHERE bulan='$cols' AND tahun='$tahun_tsrko' AND parent_srko='$r[id_srko]'"));
											
											$count = mysql_fetch_array(mysql_query("select COUNT(realisasi) as jumlah from progress_srko where parent_srko='$r[id_srko]' AND tahun='$tahun_tsrko' AND bulan='$cols'"));
											...
											if($row==1){ 
												if($cols>=1 AND $cols<=12){
													...
													if($jr['jenis_resume']==1){  // Bulan Terakhir 	
														$jr1 = mysql_fetch_array(mysql_query("SELECT sum(realisasi) as realisasi FROM progress_srko WHERE bulan='$jrbul[bln]' AND tahun='$tahun_tsrko' AND parent_srko='$r[id_srko]'"));
														
														$nilai = desimal3($jr1['realisasi']);
														$realprog = $jr1['realisasi'];
													}elseif($jr['jenis_resume']==2){// Komulatif
														$cek = mysql_fetch_array(mysql_query("SELECT jenisAktf FROM wbs WHERE tahun='$tahun_tsrko' AND parent_srko='$r[id_srko]' AND level='4' "));
														if($cek['jenisAktf']==2){
															if($jrbul['bln']==1){
																$bln = $jrbul['bln'];
															}else{
															...
															$jr2 = mysql_fetch_array(mysql_query("SELECT pencapaian FROM progress_srko WHERE bulan='$bln' AND tahun='$tahun_tsrko' AND parent_srko='$r[id_srko]'"));
															$last = $jr2['pencapaian'];
															if($last >= 100){
																$nilai = 100;
																$realprog = 100;
															}else{
																$jr2 = mysql_fetch_array(mysql_query("SELECT SUM(realisasi) as sumrealisasi FROM progress_srko WHERE (bulan BETWEEN '1' AND '$jrbul[bln]') AND tahun='$tahun_tsrko' AND parent_srko='$r[id_srko]'"));
														...
														$jr3 = mysql_fetch_array(mysql_query("SELECT AVG(realisasi) as avgrealisasi FROM progress_srko WHERE (bulan BETWEEN '1' AND '$jrbul[bln]') AND tahun='$tahun_tsrko' AND parent_srko='$r[id_srko]'"));
														$nilai = desimal3($jr3['avgrealisasi']);
														$realprog = $jr3['avgrealisasi'];
													}elseif($jr['jenis_resume']==4){	//Profit Margin
														$pm = mysql_fetch_array(mysql_query("SELECT SUM(realisasi_pm) as sumpm FROM progress_srko WHERE (bulan BETWEEN '1' AND '$jrbul[bln]') AND tahun='$tahun_tsrko' AND parent_srko='$r[id_srko]'"));
