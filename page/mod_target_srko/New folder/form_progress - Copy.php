...
?>
		...
		<form method="POST" action="page/mod_target_srko/query_progress.php">
					...
					<?php
						...
						echo"</tr>
							<tr bgcolor='#ffffb3'>
								<td>R</td>";
								for($i=1;$i<=12;$i++){
									$qprog = mysql_query("SELECT realisasi,jenis_pencapaian,realisasi_pm FROM progress_srko WHERE bulan='$i' AND tahun='$tahun_tsrko' AND id_srko='$r[id_srko]'");
									...
									if($prog['jenis_pencapaian']==3){
										...
										if($r['hasil_akhir']=="H"){
											...
											$qgca = mysql_query("SELECT progress_pgca.realisasi,progress_pgca.pencapaian
																		FROM
																		progress_pgca
																		INNER JOIN wbs ON wbs.id = progress_pgca.id AND wbs.parentId = progress_pgca.parentId AND wbs.tahun = progress_pgca.tahun
																		WHERE progress_pgca.`level`='4' AND `progress_pgca`.tahun='$tahun_tsrko' AND `progress_pgca`.cc='$cc' AND bulan='$i' AND wbs.id_srko='$r[id_srko]' AND wbs.hasil_akhir='P'
																		");
											$qreal = mysql_fetch_array($qgca);
											// $real = $qreal['realisasi'];
											$real = $qreal['pencapaian'];
										}
										// if($prog['realisasi'] == "" OR $prog['realisasi'] == 0){
											// $qgca = mysql_query("SELECT progress_pgca.realisasi
																		// FROM
																		// progress_pgca
																		// INNER JOIN wbs ON wbs.id = progress_pgca.id AND wbs.parentId = progress_pgca.parentId AND wbs.tahun = progress_pgca.tahun
																		// WHERE progress_pgca.`level`='4' AND `progress_pgca`.tahun='$tahun_tsrko' AND `progress_pgca`.cc='$cc' AND bulan='$i' AND wbs.id_srko='$r[id_srko]' AND wbs.hasil_akhir='P'
										...
										echo"<td width='5%' align='center'><input type='text' name='realisasi[]' value='$real' size='5' $disable></td>";
								...
								for($i=1;$i<=12;$i++){
									...
									if($prog['jenis_pencapaian']==3){
											echo"<td width='5%' align='center'>
												...
												<input type='text' name='hpp[]' value='$prog[hpp]' size='5' readonly>									
										</td>";
									}else{
										echo"<td width='5%' align='center'>
												<input type='hidden' name='pencapaian[]' value='$prog[pencapaian]' size='5'>
			...
			<h6><b> -> Jika Belum Ada Realisiasi Sasaran Kejra pada Bulan Tertentu Harap Di ISI dengan Angka 0 (NOL)</b></h6>
			<hr>
