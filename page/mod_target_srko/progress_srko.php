...
?>
		...
		?>
			<?php
			$bulan		= date('m')-1;
			// $bulan = mysql_fetch_array(mysql_query("SELECT COUNT(DISTINCT bulan) as bulan from progress_srko_detile where cc='$unit[CostCenter]' AND realisasi!='0.00'"));
			if($getInsert==1){
				echo"<a href='?page=form_progress&unit=".ec($unit['CostCenter'])."' class='btn btn-primary'><i class='fa fa-plus'></i> Input Progress</a>";
				
				// echo" <a href='?page=form_progress&unit=".ec($unit['CostCenter'])."' class='btn btn-warning' target='_blank'><i class='fa fa-plus'></i> Margin & HPP </a>";
				
				echo" <a href='?page=grafik_realisasi&unit=".ec($unit['CostCenter'])."&tahun=".ec($tahun_tsrko)."&bulan=".ec($bulan)."' class='btn btn-primary' target='_blank'><i class='fa fa-bar-chart'></i> Grafik Pencapaian</a>";
			}
					...
					<?php
						...
						while($r=mysql_fetch_array($query)){
							...
							<tr>
								...
								";
								$jr = mysql_fetch_array(mysql_query("SELECT DISTINCT jenis_resume FROM progress_srko_detile WHERE tahun='$tahun_tsrko' AND id_srko='$r[id_srko]'"));
								...
								$jrbul = mysql_fetch_array(mysql_query("SELECT COUNT(realisasi) as bln FROM progress_srko_detile WHERE tahun='$tahun_tsrko' AND id_srko='$r[id_srko]' AND realisasi!='0.00'"));
								...
								for($i=1;$i<=13;$i++){
									...
									if($i==13){
										if($jr['jenis_resume']==1){	
											$jr1 = mysql_fetch_array(mysql_query("SELECT sum(target) as target FROM progress_srko_detile WHERE bulan='$jrbul[bln]' AND tahun='$tahun_tsrko' AND id_srko='$r[id_srko]'"));
											$tot_target =desimal3($jr1['target']);
										
										}elseif($jr['jenis_resume']==2){
											$jr2 = mysql_fetch_array(mysql_query("SELECT SUM(target) as sumtarget FROM progress_srko_detile WHERE (bulan BETWEEN '1' AND '$jrbul[bln]') AND tahun='$tahun_tsrko' AND id_srko='$r[id_srko]'"));
											$tot_target =desimal3($jr2['sumtarget']);
										
										}elseif($jr['jenis_resume']==3){
											$jr3 = mysql_fetch_array(mysql_query("SELECT AVG(target) as avgtarget FROM progress_srko_detile WHERE (bulan BETWEEN '1' AND '$jrbul[bln]') AND tahun='$tahun_tsrko' AND id_srko='$r[id_srko]'"));
											$tot_target =desimal3($jr3['avgtarget']);
										
										}elseif($jr['jenis_resume']==4){
											$jr4 = mysql_fetch_array(mysql_query("SELECT AVG(target) as avgtarget FROM progress_srko_detile WHERE (bulan BETWEEN '1' AND '$jrbul[bln]') AND tahun='$tahun_tsrko' AND id_srko='$r[id_srko]'"));
								...
								for($row=1;$row<=2;$row++){
									...
									echo"<tr bgcolor='$rowcolor'>
										...
										for($cols=1;$cols<=13;$cols++){
											...
											$prog = mysql_fetch_array(mysql_query("SELECT pencapaian,jenis_pencapaian,jenis_resume,target,realisasi,margin FROM progress_srko_detile WHERE bulan='$cols' AND tahun='$tahun_tsrko' AND id_srko='$r[id_srko]'"));
											...
											if($row==1){ 
												if($cols>=1 AND $cols<=12){
													...
													if($jr['jenis_resume']==1){  // Bulan Terakhir 	
														$jr1 = mysql_fetch_array(mysql_query("SELECT sum(realisasi) as realisasi FROM progress_srko_detile WHERE bulan='$jrbul[bln]' AND tahun='$tahun_tsrko' AND id_srko='$r[id_srko]'"));
														...
														if($cek['jenisAktf']==2){
															...
															$jr2 = mysql_fetch_array(mysql_query("SELECT pencapaian FROM progress_srko_detile WHERE bulan='$bln' AND tahun='$tahun_tsrko' AND id_srko='$r[id_srko]'"));
															$last = $jr2['pencapaian'];
															if($last >= 100){
																$nilai = 100;
																$realprog = 100;
															}else{
																$jr2 = mysql_fetch_array(mysql_query("SELECT SUM(realisasi) as sumrealisasi FROM progress_srko_detile WHERE (bulan BETWEEN '1' AND '$jrbul[bln]') AND tahun='$tahun_tsrko' AND id_srko='$r[id_srko]'"));
														...
														$jr3 = mysql_fetch_array(mysql_query("SELECT AVG(realisasi) as avgrealisasi FROM progress_srko_detile WHERE (bulan BETWEEN '1' AND '$jrbul[bln]') AND tahun='$tahun_tsrko' AND id_srko='$r[id_srko]'"));
														$nilai = desimal3($jr3['avgrealisasi']);
														$realprog = $jr3['avgrealisasi'];
													}elseif($jr['jenis_resume']==4){	//Profit Margin
														$pm = mysql_fetch_array(mysql_query("SELECT SUM(realisasi_pm) as sumpm FROM progress_srko_detile WHERE (bulan BETWEEN '1' AND '$jrbul[bln]') AND tahun='$tahun_tsrko' AND id_srko='$r[id_srko]'"));
														$hpp = mysql_fetch_array(mysql_query("SELECT SUM(hpp) as sumhpp FROM progress_srko_detile WHERE (bulan BETWEEN '1' AND '$jrbul[bln]') AND tahun='$tahun_tsrko' AND parent_srko='$r[id_srko]'"));
											 ...
											 //Pencapaian	
												...
												if($hasil <= 0 AND ($cols>=1 AND $cols<=12)){
													...
													$prog2 = mysql_fetch_array(mysql_query("SELECT distinct jenis_pencapaian FROM progress_srko_detile WHERE tahun='$tahun_tsrko' AND id_srko='$r[id_srko]'"));
													if($prog2['jenis_pencapaian']==1){
														...
														$pm = mysql_fetch_array(mysql_query("SELECT SUM(realisasi_pm) as rpm, SUM(hpp) as hpp FROM progress_srko_detile WHERE (bulan BETWEEN '1' AND '$jrbul[bln]') AND tahun='$tahun_tsrko' AND id_srko='$r[id_srko]'"));
